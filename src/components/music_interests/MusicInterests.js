            ...
            <Tabs defaultActiveKey="artists">
                <TabPane className="artists-section sub-section" tab="Artists" key="artists">
                    {artistsList.length !== 0 ? artistsList.map((item, index) => {
                        return (<Artist key={index} artist={item.artist} imgSrc={item.src} artistsList={artistsList} index={index} removeFn={RemoveItem} removeArtist={removeArtist} />)
                ...
                <TabPane className="tracks-section sub-section" tab="Tracks" key="tracks">
                    {tracksList.length !== 0 ? tracksList.map((item, index) => {
                        return (<Track key={index} artist={item.artist} trackName={item.trackName} imgSrc={item.imgSrc} tracksList={tracksList} index={index} removeFn={RemoveItem} removeTrack={removeTrack} />)
                ...
                <TabPane className="albums-section sub-section" tab="Albums" key="albums">
                    {albumsList.length !== 0 ? albumsList.map((item, index) => {
                        return (<Album key={index} artist={item.artist} albumName={item.albumName} imgSrc={item.imgSrc} albumsList={albumsList} index={index} removeFn={RemoveItem} removeAlbum={removeAlbum} />)
